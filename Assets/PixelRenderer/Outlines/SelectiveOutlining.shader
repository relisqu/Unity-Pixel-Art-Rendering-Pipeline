Shader "PixPipe/SelectiveOutline"
{
    Properties
    {
        ...
        _InnerOutlineWidth ("Inner Outline Width", float) = 1.0
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float2 innerOffsets[4] = {
                    float2(0.0, _InnerOutlineWidth * _PixelSize),
                    float2(0.0, -_InnerOutlineWidth * _PixelSize),
                    float2(_InnerOutlineWidth * _PixelSize, 0.0),
                    float2(-_InnerOutlineWidth * _PixelSize, 0.0)
                };
                for (int j = 0; j < 4; ++j)
                {
                ...
                for (int j = 0; j < 4; ++j)
                {
                    innerUvs[j] = cord + pixelateCoord(innerOffsets[j]);
                }
                bool needsBorder = tex2D(_OutlineBuffer, cord) > 0;
                for (int j = 0; j < 4; ++j)
                {
                    float3 color = tex2D(_OutlineBuffer, uvs[j]);
                    if (color.x > 0.00001 || color.y > 0.00001 || color.z > 0.00001) needsBorder = true;
                }
                if (!needsBorder) return tex2D(_MainTex, i.uv);
                ...
                for (int j = 0; j < 4; ++j)
                {
                    normals[j] = getNormals(innerUvs[j], 0);
                    normalsDiff += normal - normals[j];
                }

                float normalsEdge = step(_NormalThreshold, normalsDiff);

                float4 objOutlineColor = tex2D(_OutlineBuffer, cord);
                if (depthEdge > 0)
                {
                ...
                if (normalsEdge > 0)
                {
                    float4 _innerOutlineColor = objOutlineColor + 0.5f * _InnerOutlineColor;
